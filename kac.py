...
class KacRandomWalk_(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, dim, n_steps):
        x = kac_kernel.parallel_kac_random_walk(x, dim, n_steps)
        ctx.dim = dim
        ctx.n_steps = n_steps
        ctx.mark_dirty(x)
        return x
    ...
    def backward(ctx, grad_x):
        grad_x = kac_kernel.parallel_kac_random_walk_bwd(grad_x.clone(), ctx.dim, ctx.n_steps)
